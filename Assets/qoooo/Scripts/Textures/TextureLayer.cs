...
{
    public class TextureLayer : MonoBehaviour, IParameterContainer, IMidiParameterContainer
    {
        [SerializeField] private MonoBehaviour _sourceComponent;
        [SerializeField] private TextureEffect _effect;
        [SerializeField] private BoolParameter _useChromaKey = new("texture.layer.chroma-key", "Use Chroma Key", true);
        [SerializeField] private EffectParams _effectParams;

        public string DisplayName => gameObject.name;
        public bool IsVisible => gameObject.activeSelf;
        public bool UseChromaKey => _useChromaKey.Value;
        ...
        public IEnumerable<IMidiBindableParameter> MidiParameters
        {
            get
            {
                yield return _useChromaKey;
                if (_sourceComponent is not IMidiParameterSource source) yield break;

                foreach (var parameter in source.MidiParameters)
        ...
        public Element CreateParameterElement()
        {
            return UI.Column(
                UI.Fold("Layer",
                    ...
                    UI.Field("Use Chroma Key", () => _useChromaKey.Value, value => _useChromaKey.TrySetValue(value))
                ).Open(),
                UI.Fold("Effects",
